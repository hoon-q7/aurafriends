...
lucky: {
colors: colorNames,  // ✅ 수정: hex → 색상 이름
...
// ════════════════════════════════════════════════════════
// ════════════════════════════════════════════════════════
//  한자 사전 — 기본값 하드코딩 (Supabase 없어도 동작)
//  Supabase 연결 시 자동으로 DB 데이터로 덮어씀
// ════════════════════════════════════════════════════════
let HANJA_DICT = {
// ── 성(姓) — 빈도 높은 순 ──
...
const cfg = window.__supabaseConfig;
if (!cfg) {
...
console.warn(“Supabase 연결 실패 — 내장 사전 유지:”, e.message);
}
}
...
// ✅ 수정: 한자 유니코드 범위로 직접 검증 (기존 \u3131-\uD79D는 한자도 포함해버려서 버그)
if (!/[\u4E00-\u9FFF\u3400-\u4DBF\uF900-\uFAFF]/.test(h)) {
