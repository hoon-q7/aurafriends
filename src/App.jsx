...
// 천간별 동물 기반 속성
const _SA = [
    ["곰돌이","🐻",0,"#0F5132","#F0FFF4","강한한 목 (木)의 기운",["우직함","신뢰","창의"],{col:"40대 이후 명예와 재물이 쌓이는 대기만성형.","고집보다 유연함이 기회를 부릅니다."}],
    ["토끼","🐰",0,"#00695C","#F5FFFF","유연한 목 (木)의 기운",["친화력","감수성","적응"],{col:"인복이 강하여 협력자를 통해 전성기를 맞이합니다.","결단력이 필요한 순간엔 침착함이 핵심입니다."}],
    ...
    ["강아지","🐶",2,"#827717","#FFFDF0","견고한 토 (土)의 기운",["신뢰","안정","성실"],{col:"중년에 안정적인 자산 운과 평온을 누림.","새로운 변화에 마음을 열어야 기회가 옵니다."}],
    ["돼지","🐷",2,"#6D4C41","#FAF6F0","풍요로운 토 (土)의 기운",["풍요","포용","복록"],{col:"평생 먹을 복과 재물운이 끊이지 않는 복록.","실속을 챙기는 지혜가 필요합니다."}],
    ["늑대","🐺",3,"#455A64","#F8F8F8","결단력 있는 금 (金)의 기운",["결단력","정의감","전문성"],"전문직 분야에서 큰 두각을 나타내며 대운이 옵니다.","부드러운 말투가 행운의 통로입니다."],
    ["햄스터","🐹",3,"#1565C0","#F5F5FF","명석한 금 (金)의 기운",["지혜","분석력","전략"],{col:"명석한 지혜가 재물이 되는 운명.","자만하면 큰 손해가 오니 겸손이 필수입니다."}],
    ["고래","🐳",4,"#1976D2","#F0FAFF","넓은 수 (水)의 기운",["통찰","비전","포용"],{col:"해외운·유통운이 강해 넓은 무대에서 성공.","빠른 실행력이 운을 보강합니다."}],
    ["병아리","🐥",4,"#303F9F","#F2F8FF","많은 수 (Water)의 기운",["영감","순수","소통"],{col:"예술·교육 분야에서 큰 성취를 이룹니다.","금전 거래·계약 시 꼼꼼해야 합니다."}],
];
...
const _BSTM_KO = ["수 (水)", "토 (土)", "목 (木)", "목 (木)", "토 (土)", "화 (火)", "화 (火)", "토 (土)", "금 (金)", "금 (金)", "수 (水)", "토 (土)", "목 (木)", "목 (木)", "토 (土)", "화 (火)", "화 (火)", "토 (土)", "금 (金)", "금 (金)"];
    ...
    <>
      <div className="bg-white rounded-[30px] p-5 shadow-sm" style={{border: "1.5px solid #1C3A33"}}>
        ...
        <div className="mt-3 pt-3 border-t border-gray-50 grid grid-cols-2 gap-2">
          {[...cal.good].slice(0,1).map(d=><div key={d} className="text-center"><span className="text-emerald-600">✨ 길일</span> {d}일</div>)}
          {[...cal.bad].slice(-1).map(d=><div key={d} className="text-center"><span className="text-red-600">△ 흉일</span> {d}일</div>)}
        </div>
      </div>
      <AnimatePresence>
    ...
    <motion.div key="home" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="p-5">
      ...
      {p&&char?(
        ...
        <motion.div initial={{scale:.95,opacity:0}} animate={{scale:1,opacity:1}} className="text-center py-20 bg-white rounded-[40px] cursor-pointer" style={{border:"2px dashed #ddd"}} onClick={onAdd}>
          ...
          <p className="text-sm text-gray-300 mt-1">생년월일을 입력하면 120가지 동물 중 L</p>
            ...
            {profiles.map(p=>{
              ...
              return(
                <motion.div key={p.id} layout initial={{opacity:0,x:-15}} animate={{opacity:1,x:0}} exit={{opacity:0,x:15}} className="flex items-center gap-3 bg-white p-4 rounded-[22px]" style={{border:`2px solid ${on?c.color:"#f0f0f0"}`}}>
                  <motion.button whileTap={{scale:.9}} onClick={()=>onSelect(p.id)} className="flex-1 min-w-0 cursor-pointer flex items-center gap-3" onClick={()=>onSelect(p.id)}>
