...
const [ko,icon,elem,color,bg,essence,keywords,luckTrend,caution,colorNames] = _SA[s];
...
lucky: {
colors: colorNames,
numbers: [s+1, s+6].map(n => n > 10 ? n-10 : n),
...
{text:"日Just月將",reading:"일취월장",meaning:"날마다 달마다 성장한다 — 오늘도 어제보다 한 뼘 성장 중입니다."},
...
if (!supabase) {
console.log("Supabase 미설정 — 내장 한자 사전 사용");
return;
}
try {
...
// Supabase 데이터로 덮어씀 (기본 사전 위에 추가/업데이트)
data.forEach(row => {
...
<input type="number" value={customStrokes} placeholder="예) 12"
...
className="w-full py-2.5 rounded-2xl font-bold text-white mt-2"
style={{background:"#1C3A33"}}>한자 등록하기</button>
</div>
</motion.div>
)}
</AnimatePresence>
